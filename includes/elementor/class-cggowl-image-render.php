...
class Cggowl_Featured_Image_Getter extends Group_Control_Image_Size
{
    ...
    public static function get_attachment_image_html_generator($attachment_id, $settings, $image_size_key = 'image', $image_key = null,$extra = '')
    {
        ...
        if (!empty($image['id']) && in_array($size, $image_sizes)) {
            ...
            if (!empty($image_src)) {
                ...
                $html .= sprintf('<img class="swiper-lazy" src="%s" title="%s" alt="%s"%s />', esc_attr($image_src), Control_Media::get_image_title($image), Control_Media::get_image_alt($image), $image_class_html);
            }
        }
        /**
         * Get Attachment Image HTML
    ...
    protected function init_fields()
    {
        $fields         = [];
        $fields['size'] = [
            'label'       => esc_html_x('Image Size', 'Image Size Control', 'cggowl'),
            'type'        => Controls_Manager::SELECT,
            'label_block' => false,
        ];
        $fields['custom_dimension'] = [
            'label'       => esc_html_x('Image Dimension', 'Image Size Control', 'cggowl'),
            'type'        => Controls_Manager::IMAGE_DIMENSIONS,
            'description' => esc_html__('You can crop the original image size to any custom size. You can also set a single value for height or width in order to keep the original size ratio.', 'cggowl'),
            'condition'   => [
                'size' => 'custom',
            ],
            'separator'   => 'none',
    ...
    private function get_image_sizes()
    {
        ...
        $image_sizes['full'] = esc_html_x('Full', 'Image Size Control', 'cggowl');
        if (!empty($args['include']['custom']) || !in_array('custom', $args['exclude'])) {
            $image_sizes['custom'] = esc_html_x('Custom', 'Image Size Control', 'cggowl');
        }
        return $image_sizes;
    }
    /**
